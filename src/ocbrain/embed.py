...
from ocbrain.judge import scope_tag_for_privacy
...
# --------------------------------------------------------------------------- #
# Vector codec — float32 blobs (stdlib only, no numpy)
# --------------------------------------------------------------------------- #
def encode_embedding(vector: list[float]) -> bytes:
    """Pack a float vector into a ``float32`` byte blob for the ``embedding`` BLOB."""
    return array("f", (float(x) for x in vector)).tobytes()


def decode_embedding(blob: bytes | memoryview | None) -> list[float]:
    """Unpack a ``float32`` byte blob back into a list of floats.

    Returns ``[]`` for a NULL/empty blob so callers can treat "no embedding" and
    "empty embedding" identically (both fall back to lexical scoring).
    """
    if not blob:
        return []
    arr = array("f")
...
def knowledge_text(row: sqlite3.Row | Mapping[str, Any]) -> str:
    """Compose the embeddable text for a knowledge row (title/subject/predicate/value)."""
