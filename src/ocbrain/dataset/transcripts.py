...
_ENVELOPE_MARKER = "Conversation info (untrusted metadata):"
_ENVELOPE_JSON_RE = re.compile(r"```json\s*(\{.*?\})\s*```", re.DOTALL)
...
def parse_telegram_envelope(text: str) -> tuple[dict | None, str]:
    """Extract the untrusted-metadata JSON and the human message after it.

    Returns ``(envelope_dict_or_None, message_text)``. When no envelope marker is
    present the input is returned unchanged with a ``None`` envelope.
    """
    idx = text.find(_ENVELOPE_MARKER)
    if idx == -1:
        return None, text
    after = text[idx + len(_ENVELOPE_MARKER) :]
    match = _ENVELOPE_JSON_RE.search(after)
    ...
    # Anything before the marker plus the message after the fence is the human text.
    message = (text[:idx] + remainder).strip()
...
def classify_user_text(
    ...
    *,
    ...
    if _ENVELOPE_MARKER in stripped:
