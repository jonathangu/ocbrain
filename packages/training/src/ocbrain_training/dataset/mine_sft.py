...
from ocbrain_training.dataset.quality import _REFUSAL_RE, sanitize_sft_text, store_example
...
def _exported_context(
    session: Session,
    upto: int,
    sanitized_texts: tuple[str, ...] | None = None,
) -> list[dict[str, str]]:
    """Non-injected user + assistant text turns before ``upto`` (export order)."""
    out: list[dict[str, str]] = []
    for index, turn in enumerate(session.turns[:upto]):
        if turn.role not in {"user", "assistant"} or (
            turn.role == "user" and turn.kind == "injected"
        ):
            continue
        text = (
            sanitized_texts[index] if sanitized_texts is not None else sanitize_sft_text(turn.text)
        )
        if not text:
            continue
        if turn.role == "user":
...
def segment_exchanges(session: Session, cfg: OcbrainConfig | None = None) -> list[Exchange]:
    ...
    ds = cfg.dataset
    turns = session.turns
    # Long-lived sessions may contain thousands of exchanges. Sanitizing the
    # whole prefix once per target turns secret scanning into O(n²) work. Cache
    # each model-visible turn exactly once for this segmentation pass.
    sanitized_texts = tuple(
        sanitize_sft_text(turn.text)
        if turn.role in {"user", "assistant"}
        and not (turn.role == "user" and turn.kind == "injected")
        else ""
        for turn in turns
    )
    n = len(turns)
    ...
    for gi, u in enumerate(user_idxs):
        ...
        for k in range(u + 1, end):
            t = turns[k]
            if t.role == "assistant" and len(sanitized_texts[k]) >= ds.sft_min_assistant_chars:
                target_idx = k
        ...
        context = _head_trim(
            _exported_context(session, target_idx, sanitized_texts),
        ...
        exchanges.append(
            Exchange(
                ...
                target_text=sanitized_texts[target_idx],
                trigger_idx=u,
...
def _next_user_text(session: Session, after: int) -> str | None:
    for turn in session.turns[after + 1 :]:
        if turn.role == "user" and turn.kind != "injected":
            return sanitize_sft_text(turn.text)
    return None
