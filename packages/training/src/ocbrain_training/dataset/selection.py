...
def _currently_eligible(
    ...
    *,
    ...
    """Recheck a derived row instead of trusting stale class/grade columns."""
    if str(row["dataset"]) != dataset:
        return False
    current_class, _reason = classify_record(row)
    return current_class == train_class
    ...
    for dataset, train_class in PACK_CLASSES.items():
        rows = conn.execute(
            ...
            SELECT id, dataset, source_kind, privacy_scope, quality_label,
                   quality_reasons, example_json, train_class
        ...
        eligible = [
            row
            for row in rows
            if _currently_eligible(row, dataset=dataset, train_class=train_class)
        ]
        selected[dataset] = sorted(eligible, key=lambda row: _rank(seed, dataset, row))[
            : wanted[dataset]
        ]
        ...
        for rank, row in enumerate(rows, 1):
            conn.execute(
                ...
                SET train_selected = 1, train_selection_rank = ?,
                    train_selection_reason = 'deterministic_quality_pack_v2',
                    train_selected_at = ?
                WHERE id = ?
                """,
    ...
    *,
    ...
    for dataset, train_class in PACK_CLASSES.items():
        rows = conn.execute(
            ...
            SELECT id, dataset, source_kind, privacy_scope, quality_label,
                   quality_reasons, example_json, train_class,
                   train_selection_rank
            FROM dataset_examples
            WHERE dataset = ? AND train_class = ? AND train_selected = 1
              AND quality_label = 'good' AND privacy_scope != 'private'
              AND instr(COALESCE(quality_reasons, ''), 'injection_flagged') = 0
              AND grade_score >= ?
    ...
    for _dataset, rows in chosen.items():
        for rank, row in enumerate(rows, 1):
            conn.execute(
                ...
                UPDATE dataset_examples
                SET train_selected = 1, train_selection_rank = ?,
                    train_selection_reason = 'deterministic_passing_pack_v2',
                    train_selected_at = ?
                WHERE id = ?
                """,
                (rank, timestamp, row["id"]),
